...
class Pix2PixModel(torch.nn.Module):
    ...
    def __init__(self, opt):
        ...
        print('netG:', opt.norm_G)
        print('netD:', opt.norm_D)
        print('netE:', opt.norm_E)
        # set loss functions
        if opt.isTrain:
...
=======
                    self.criterionVGG = networks.InceptionLoss(self.opt.gpu_ids)                
>>>>>>> da39c358d95303aff4abee381eaf94befed3de1f
                else:
    ...
    def forward(self, data, epoch=0, mode=None):
        input_semantics, real_image = self.preprocess_input(data)

        if mode == 'generator':
            g_loss, generated = self.compute_generator_loss(input_semantics, real_image, epoch)
            return g_loss, generated
        elif mode == 'discriminator':
            d_loss = self.compute_discriminator_loss(input_semantics, real_image, epoch)
            return d_loss
...
>>>>>>> da39c358d95303aff4abee381eaf94befed3de1f
            if self.opt.inception_loss:
                G_losses['Inception'] = self.criterionVGG(fake_image, real_image) \
                    * self.opt.lambda_vgg
            else:
    ...
    def discriminate(self, input_semantics, fake_image, real_image, epoch):
        if not type(fake_image) == list:
            if len(self.opt.diff_aug) > 0:
                real_image, fake_image, input_semantics = DiffAugment(real_image,
                                                                      fake_image,
                                                                      input_semantics,
                                                                      policy=self.opt.diff_aug)
            ...
            for i in range(len(fake_image) - 1, -1, -1):
                if len(self.opt.diff_aug) > 0:
                    generated_image, real_image, input_semantics = DiffAugment(fake_image[i],
                                                                               real_image,
                                                                               input_semantics,
                                                                               policy=self.opt.diff_aug)
                else:
