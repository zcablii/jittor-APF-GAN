...
class DepthManager(Plugin):
    ...
    def __init__(self,
                 ...
                 max_depth,
                 minibatch_default,
                 minibatch_overrides={3:80, 4:64, 5:48, 6:36, 7:24,  8:16},
        ...
        self.lod_training_nimg = stabilize_epoch * 10000
        self.lod_transition_nimg = fade_epoch *10000
        self.trainer = None
        self.depth = -1
        self.alpha = -1
        ...
        # print('DepthManager max_depth: ', self.max_depth)
    ...
    def iteration(self, *args):
        ...
        if self.trainer.cur_nimg  >= self.trainer.tick_start_nimg + self.trainer.tick_duration_nimg:
            minibatch_size = self.minibatch_overrides.get(depth, self.minibatch_default)
            self.trainer.dataiter = iter(self.create_dataloader(dataset, self.params, minibatch_size))
        if depth != self.depth:
            print('switch depth: ', depth)
            ...
            self.trainer.tick_duration_nimg = self.tick_kimg_default * 1000
