import jittor as jt
from jittor import init, unsqueeze
...
model = models_mae.__dict__[opt.model](img_size = (opt.input_hight, opt.input_width), norm_pix_loss=False, img_styles = len(img_id_table)) 
...
# optimizer_G = torch.optim.AdamW(param_groups, lr=opt.lr, betas=(0.9, 0.95)) 
optimizer_D = jt.optim.AdamW(discriminator.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2))
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, (real_img, real_label, img_ids, idxs) in enumerate(dataloader):
        ...
        fake_img_jt = jt.array(np.array(fake_img_tensor.detach().cpu()))
        
     
        # print(real_img.shape, real_label.shape, fake_img_jt.shape,fake_img_tensor.shape)
        # ---------------------
        #  Train Discriminator
        # ---------------------
        start_grad(discriminator)
        fake_AB = jt.contrib.concat((real_label, fake_img_jt), 1) 
        pred_fake = discriminator(fake_AB.detach())
        loss_D_fake = criterion_GAN(pred_fake, False)
        real_AB = jt.contrib.concat((real_label, real_img), 1)
        ...
        loss_G_L1 = criterion_pixelwise(fake_img_jt, real_img)
        loss_G = loss_G_GAN + lambda_pixel * loss_G_L1

        jt_grad = jt.grad(loss_G,[fake_img_jt])
        jt_grad = torch.tensor(jt_grad[0].numpy()).cuda()

        jt2torFun = jt2tor()

        fake_img_tensor_with_g = jt2torFun.apply(fake_img_tensor, jt_grad)
        fake_img_tensor_with_g.sum().backward()
        optimizer_G.step()
