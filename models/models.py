...
from models.spade.generator import SPADE_Generator
...
class OASIS_model(nn.Module):
    def __init__(self, opt):
        ...
        if opt.generator_name == 'oasis':
            self.netG = generators.OASIS_Generator(opt)
        elif opt.generator_name == 'spade':
            self.netG = SPADE_Generator(opt)
        if opt.phase == "train":
    ...
    def load_checkpoints(self):
        ...
        elif self.opt.continue_train and misc.is_main_process():
    ...
    def init_networks(self):
        ...
        if self.opt.phase == "train":
            if self.opt.generator_name == 'spade':
                networks = [self.netD]
            else:
                networks = [self.netG, self.netD]
        else:
...
def put_on_multi_gpus(model, opt):
    if opt.gpu_ids != "-1":
        if opt.distributed:
            model.cuda()
            model = nn.parallel.DistributedDataParallel(model, device_ids=[opt.gpu], find_unused_parameters=True)
        else:
